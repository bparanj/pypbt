...
2. Run the tests with `pytest pbt101.py` - if they don't fail, go to step 1.
...
def __eq__(self, other):
    ...
    if self.value is None or other.value is None:
        return False
    if isinstance(self.value, float) and math.isnan(self.value) and isinstance(other.value, float) and math.isnan(other.value):
        return True
...
def test_sort_a_list_basic():
    ...
    assert sort_a_list([0, -1, 1, -2, 2]) == [-2, -1, 0, 1, 2]  # Test with negative numbers and zero
    assert sort_a_list([3.2, 1.1, 2.3]) == [1.1, 2.3, 3.2]  # Test with non-integer numbers
...
def leftpad(string, width, fillchar):
    ...
    padding = fillchar * max(0, width - len(string))  # calculate the padding
    return padding + string  # prepend the padding to the string
...
def test_leftpad(string, width, fillchar):
    ...
    assert 0 <= width <= 1000
    assert leftpad('cat', width=5, fillchar="Z") == 'ZZcat'
    assert leftpad('Dog', width=2, fillchar="Z") == 'Dog'
...
class Record(object):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        return self._deep_equals(self.value, other.value)

    def _deep_equals(self, a, b):
        ...
        if isinstance(a, list) and isinstance(b, list):
            ...
            for item_a, item_b in zip(a, b):
                if not self._deep_equals(item_a, item_b):
                    return False
            return True
